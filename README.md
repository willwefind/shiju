...
> 🩹 **装不上？对号入座：**
>
> **① 点了链接，浏览器却把它当文件下载了，篡改猴还说「用户脚本无效」**
> —— 新版 Chrome / Edge 的扩展机制下，`.user.js` 常常是被浏览器直接下载，
> 而不是交给篡改猴。Edge 还会跟一句「**可能会损害你的设备**」，那只是它不认识
> 没签名的 `.js` 文件，跟脚本本身没关系。
> **先点「保留」** —— 在你点之前，文件其实还没真正落盘，篡改猴去读只能读到空的，
> 于是报「用户脚本无效」。保留之后，去下载目录**把 `shiju.user.js` 拖进浏览器窗口**，
> 篡改猴就会弹出安装页。
>
> **② 安装页上的「安装 / Overwrite」点了没反应**
> —— 先去篡改猴面板看一眼版本号，有时候它其实装上了，只是那个页面不给回音。
>
> **③ 以上都不行 —— 这条从来不会卡：**
> 篡改猴面板 → **＋（新建脚本）**（要是旧的还在，就点开它选**编辑**）→
> 编辑器里**全选删掉** → 把 [`shiju.user.js`](shiju.user.js) 的内容整个粘进去 → `Ctrl+S`。

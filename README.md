...
**但这要两边都是 v0.16.4 以上才管用** —— 旧版不认识这面旗子，也不会插旗子，
于是它照跑不误，酒馆那一页就成了两颗「摘」、两个面板。

**怎么办**（任选）：

1. **把篡改猴里那份更新到 v0.16.4 以上** ——
   篡改猴面板 → 拾句 → **检查更新**（脚本头里有 `@updateURL`，它自己也会定期查）。
   两边都新了之后就不用管了。
2. **把篡改猴里那份关掉** —— 反正酒馆那一页有扩展那份就够了。
   点篡改猴图标，把「拾句」的开关关掉。**代价是别的网页上也就没有了。**

> ⚠️ 说句实在的：**第 1 条我们只在同一个页面环境里验过**（两份都由页面加载）。
> 篡改猴是把脚本跑在浏览器的「隔离环境」里的 —— 按理说那面旗子插在 DOM 上、两边都看得见，
> 但**我们没有在真的「篡改猴 + 酒馆扩展」双装环境下验证过它**。
> 你更新完还是出两个的话，来提个 issue，我们照实改这一节 —— 那样第 2 条就是唯一解。
...
> 扩展加载、面板挂载、选中出「摘」、开白纸、存图、以及「装两份会不会打架」。

...
> ⚠️ **这一节我们没有在真手机上走过。** 面板的手机版排布是在电脑浏览器的
> 手机模拟下量出来的（375px 宽：预览图在上、控制在下、整张卡片一眼看得全、
> 不会横向溢出）。装插件那几步是这类浏览器的通用做法。
...
然后点地址栏左边的 **ᴀA → Userscripts → 新建脚本**，粘贴进去保存。
...
**浏览器自己的页面不给任何扩展和用户脚本跑。** 包括：

- Edge 打开就看到的那个**起始页 / 新标签页**（`edge://newtab`）
- Chrome 的新标签页（`chrome://newtab`）、`about:blank` 空白页
- 设置页、扩展管理页、浏览器自带的下载页
